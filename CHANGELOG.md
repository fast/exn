...
* `ErrorExt::raise_all` supports `err.raise_all(sources)` = `Exn::raise_all(err, sources)`, which allows for raising an error with multiple sources in a more fluent way.
